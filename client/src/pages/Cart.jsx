...
const Cart = () => {
  const { products,currency,cartItems ,removeFromCart,getCartItemsCount,updateCartItem,navigate,getCartTotalAmount} = useAppContext();
  ...
  for (const key in cartItems) {
    const product = products.find((item) => item._id === key); // Ensure key matches your ID field
    if (product) {
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                <h1 className="text-3xl font-medium mb-6">
                    Shopping Cart <span className="text-sm text-primary">{getCartItemsCount()}</span>
                </h1>
